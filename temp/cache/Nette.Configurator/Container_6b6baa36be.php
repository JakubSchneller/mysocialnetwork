...
class Container_6b6baa36be extends Nette\DI\Container
{
	protected $meta = [
		'types' => [
			...
			'App\Presenters\ProfilePresenter' => [1 => ['application.4']],
			'App\Presenters\SearchPresenter' => [1 => ['application.5']],
		...
		'services' => [
			...
			'application.4' => 'App\Presenters\ProfilePresenter',
			'application.5' => 'App\Presenters\SearchPresenter',
	...
	public function createServiceApplication__4(): App\Presenters\ProfilePresenter
	{
	...
	public function createServiceApplication__5(): App\Presenters\SearchPresenter
	{
		$service = new App\Presenters\SearchPresenter($this->getService('database.default.context'));
		$service->injectPrimary($this, $this->getService('application.presenterFactory'),
			$this->getService('routing.router'), $this->getService('http.request'),
			$this->getService('http.response'), $this->getService('session.session'),
			$this->getService('security.user'), $this->getService('latte.templateFactory'));
	...
	{
		$service = new Nette\Database\Connection('mysql:host=127.0.0.1;dbname=socialnetwork;port=8889',
			'root', 'root', ['lazy' => true]);
